...
- **品牌色统一为 indigo** —— 清掉散落的 GitHub 蓝 / Apple 蓝 / cyan 混用，logo 去掉彩虹变色。建立完整 `--accent-*` 令牌体系（color/light/soft/softer/border/ring），三套主题统一引用。
- **修复 `.glass` 死类** —— 这个被 11 处引用的 class 此前根本没定义（白用）；现定义为主题自适应表面，并加 `.glass-blur` 给 sticky 头部按需开启模糊。
- **跨主题白色清理** —— 把 50+ 处 `bg-white/N` / `border-white/N` 换成主题令牌，浅色 / soft 主题不再发灰、modal 改实底 + 柔影。
- **卡片选中态 Linear 化** —— 去掉厚重 `ring-4`/`shadow-2xl`，改左侧 accent 竖条 + 单色边框 + 柔光环；meta 行去掉多色大写徽章，改单色 chip + 相对时间。
- 搜索框补齐放大镜图标 + focus 柔光环；tab / 分类胶囊从实心重块改轻盈软底色。

### 新增 Typora 主题

- 第 4 套主题 **typora**：纯白画布 + GitHub 发丝灰边框（`#e1e4e8`）+ 炭墨灰单色 accent（`#44403a`），无背景光晕，还原写作软件的纸感书卷气。
- 主题循环扩展为 dark → light → soft → **typora** → auto，顶栏图标 5 态。

### 招牌时刻 + 主张

- **实时落袋（live drop-in）** —— 窗口开着时复制任意内容，新条目从顶部滑入并伴随 accent 光环脉冲，让你"看得见"剪贴板历史在生长。
- **聚焦核心** —— Dashboard 统计降级为二级视图（默认进剪贴板历史，📊 图标 / Ctrl+D 调出）；默认时间窗从「近 1 天」改为「全部」，久未复制不再开屏空屏。

### 智能识别精准化

- **实体提取去噪** —— 电话只认真手机号（排除版本号 / 构建号 / 时间戳）、IP 排除版本号式、每类限量、超长日志不抽取，告别 chip 刷屏。
- **代码识别收紧** —— `is_code` 去掉会被中文 / 自然语言误伤的弱信号（`=>` / 裸 `;` / 孤立 `let`），改正则锚定判定；单行代码仍能识别。
- **语言高亮重写** —— `detectLanguage` 从「首个命中 + 默认 JS」改为 12 语言加权打分，无明确信号回退纯文本，不再把 Python/SQL/散文错高亮成 JS。

### 性能 & 健壮性

- **复制不再全量重载** —— `clip:created` 改增量合并（只拉最新条目去重重排），不再每次复制重拉整个历史。
- **渲染窗口化** —— 列表只挂载约 50 行、滚动增长，数据全留内存保证搜索 / `type:` / 正则不受影响。
- **容错读取** —— 一条含无效 UTF-8 的记录此前会让整个历史读取失败、列表全空；现按 lossy 降级，坏字段最多显示 `�`。
- **迁移自愈** —— 以实际列存在性为准补缺列，schema 与版本号错位时不再把 app 卡死成空列表。
- **窗口零闪烁** —— 隐藏启动 + 前端恢复几何后再显示，消除居中再跳位的闪烁（Rust 留 1.5s 安全网）。

### 其它

- 分类栏顺序调整：图片移至第 3 位、收藏移至最后。
- 新增 `detection_quality_tests` 锁住代码 / 散文判别、版本号噪声、长文档跳过等行为。
