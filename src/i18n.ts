...
const translations: Record<string, Record<Locale, string>> = {
    ...
    'search.no_match_for': { zh: '未找到匹配 "{q}" 的记录', en: 'No results for "{q}"' },
    'search.no_filter_results': { zh: '当前筛选条件下没有记录', en: 'No records under current filters' },
    ...
    'modal.bulk_delete_title': { zh: '确认批量删除', en: 'Confirm bulk delete' },
    'modal.bulk_delete_body': { zh: '将永久删除选中的 {n} 条记录，无法撤销。', en: 'This will permanently delete {n} selected clips. This cannot be undone.' },
    'modal.copy_title': { zh: '放入系统剪贴板', en: 'Put into system clipboard' },
    'modal.copy_body': { zh: '是否将选中内容覆盖到系统剪贴板中？', en: 'Overwrite the system clipboard with this clip?' },
    'modal.copy_remember': { zh: '记住选择，以后直接放入', en: 'Remember my choice and apply it next time' },
    'action.confirm_extract': { zh: '确定提取', en: 'Confirm' },
    ...
    'footer.click_copy': { zh: '双击复制', en: 'Double-click Copy' },
    ...
    'shortcut.click': { zh: '选中卡片', en: 'Select card' },
    'shortcut.dblclick': { zh: '复制到剪贴板', en: 'Copy to clipboard' },
    ...
    'settings.save_failed': { zh: '保存失败: {e}', en: 'Save failed: {e}' },
    'settings.save_button': { zh: '保存设置', en: 'Save settings' },
    'settings.hotkey_main': { zh: '全局呼出快捷键', en: 'Global show/hide hotkey' },
    'settings.hotkey_mini': { zh: '极简模式快捷键', en: 'Minimalist mode hotkey' },
    'settings.hotkey_record_tip': { zh: '点击并按下新按键组合', en: 'Click and press the new key combo' },
    'settings.hotkey_mini_tip': { zh: '设定极简模式按键', en: 'Set the minimalist mode hotkey' },
    'settings.hotkey_help': { zh: '提示: 点击右侧按钮并按下新的按键组合进行录制。', en: 'Tip: click the button on the right and press the new key combo to record.' },
    'settings.hotkey_mini_help': { zh: '应用内按此快捷键切换极简搜索模式。默认 Ctrl+M。', en: 'Press this in-app to toggle the minimalist search mode. Default Ctrl+M.' },
    'settings.recording': { zh: '录制中...', en: 'Recording…' },
    'settings.autostart': { zh: '开机自动启动', en: 'Launch on startup' },
    'settings.autostart_help': { zh: '跟随系统启动时自动运行并在后台驻留', en: 'Run automatically on system startup and stay in the background' },
    'settings.intercept_prompt': { zh: '新剪贴板内容提示', en: 'Prompt on new clipboard content' },
    'settings.intercept_prompt_help': { zh: '开启时每次在外部 Ctrl+C 都会弹窗询问是否收录', en: 'When on, every external Ctrl+C asks whether to capture the clip' },
    'settings.double_ctrl': { zh: '双击 Ctrl 呼出窗口', en: 'Double-press Ctrl to toggle window' },
    'settings.double_ctrl_help': { zh: '快速按下两次 Ctrl 键来显示或隐藏主窗口 (Windows 专享)', en: 'Quickly tap Ctrl twice to show or hide the main window (Windows only)' },
    'settings.retention_title': { zh: '自动清理历史记录', en: 'Auto-cleanup history' },
    'settings.retention_help': { zh: '注意: 置顶 📌 和 收藏 ⭐ 的内容永远不会被自动清理。', en: 'Note: pinned 📌 and favorited ⭐ clips are never auto-cleaned.' },
    ...
    'settings.ignored_title': { zh: '隐私 — 忽略的应用', en: 'Privacy — Ignored apps' },
    'settings.ignored_help': { zh: '从这些应用复制的内容永远不会被记录。', en: 'Clipboard content from these apps will never be recorded.' },
    ...
    'dash.filter_tip_body': { zh: '点击上方模块将同时应用 {range} 与分类类型过滤，主列表将即时更新。', en: 'Clicking a module applies the {range} window and the category filter simultaneously; the main list updates instantly.' },
    'dash.discovery': { zh: '智能发现', en: 'Smart Discovery' },
    'dash.settings': { zh: '设置', en: 'Settings' },
    'dash.clear': { zh: '清空', en: 'Clear' },
    'dash.category_distribution_with_range': { zh: '类别分布 ({range})', en: 'Category distribution ({range})' },
    ...
    'mini.everything_not_installed_title': { zh: '需要 Everything 才能搜索本机文件', en: 'Everything is required for file search' },
    'mini.everything_not_installed_body': { zh: '前往 voidtools.com 下载并安装 Everything 后再试。安装时请勾选「随系统启动」。', en: 'Install it from voidtools.com, then enable "Start on system boot" in its settings.' },
    'mini.everything_not_running_title': { zh: 'Everything 未运行', en: 'Everything is not running' },
    'mini.everything_not_running_body': { zh: '已检测到 Everything 已安装但未启动，请在开始菜单或托盘里启动 Everything 后重试。', en: 'Everything is installed but its background service is not running. Start it from the Start menu or system tray and try again.' },

    // ── Onboarding ──
    'onboard.s1_title': { zh: '欢迎使用 Super Clip', en: 'Welcome to Super Clip' },
    'onboard.s1_body': { zh: '你的剪贴板从此带格式、能搜图、还能找本机文件——而且全部本地运行，没账号、没遥测。', en: 'Your clipboard now keeps formatting, OCRs images, and searches local files — all on-device, no accounts, no telemetry.' },
    'onboard.s2_title': { zh: '随时呼出主窗口', en: 'Bring up the main window anytime' },
    'onboard.s2_body': { zh: '按 Ctrl+Space 显示或隐藏主窗口（可在设置里改成你习惯的组合键）。', en: 'Press Ctrl+Space to show or hide the main window. (Customizable in settings.)' },
    'onboard.s3_title': { zh: '极简搜索 — Ctrl+M', en: 'Spotlight-style search — Ctrl+M' },
    'onboard.s3_body': { zh: '弹出小窗，同时搜剪贴板历史 + 本机硬盘文件（需 Everything）。Tab 切类别，Enter 复制并自动粘贴到上一个窗口。', en: 'A pop-up that searches your clipboard history AND your whole drive (needs Everything). Tab cycles categories, Enter copies and auto-pastes into the previous window.' },
    'onboard.s4_title': { zh: '主列表里：单击选中，双击复制', en: 'In the main list: single-click selects, double-click copies' },
    'onboard.s4_body': { zh: '为了避免误触，主窗口卡片单击只是高亮选中——双击才真正复制到剪贴板。Enter 键则会复制并自动粘贴。', en: 'To avoid accidental copies, single-clicking a card only highlights it. Double-click to copy. Press Enter to copy AND auto-paste.' },
